...
type (
	type_nud_handler func(p *parser) ast.Type
	type_led_handler func(p *parser, left ast.Type, bp binding_power) ast.Type
)

type (
	type_nud_lookup map[lexer.TokenKind]type_nud_handler
	type_led_lookup map[lexer.TokenKind]type_led_handler
	type_bp_lookup  map[lexer.TokenKind]binding_power
)

var (
	type_bp_lu  = bp_lookup{}
	type_nud_lu = type_nud_lookup{}
	type_led_lu = type_led_lookup{}
)
